...
class CentrNLLWebVision(object):
    ...
    def __init__(self, root_dir: str, imagenet_root_dir: str, out_dir: str,
                 num_classes: int = 50) -> None:
    ...
    def _load_valset(self, save: bool = True) -> None:
        ...
        if os.path.exists(self.valset_path):
            ...
            imagenet_val_data = []
            class_names = [x[0] for x in sorted(self.class_to_idx.items(), key=lambda x: x[1])]
            for cls_idx in range(self.num_classes):
                class_name = class_names[cls_idx]
                img_filenames = os.listdir(os.path.join(self.imagenet_root_dir, 'val', class_name))
                for img in img_filenames:
                    img_path = os.path.join(self.imagenet_root_dir, 'val', class_name, img)
    ...
    def _load_trainset(self, save: bool = True) -> None:
        ...
        if os.path.exists(self.trainset_path):
            ...
            train_labels = {}
            train_imgs = []
            with open(os.path.join(self.root_dir, 'info/train_filelist_google.txt'), 'r') as f:
                lines = f.read().splitlines()
            for l in lines:
                img, target = l.split()
...
if __name__ == '__main__':
    centr_webvision = CentrNLLWebVision(root_dir='/Users/liangsiqi/Downloads/WebVision1.0',
