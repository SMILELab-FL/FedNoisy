...
class FedNLLClothing1M(NLLClothing1M, FedNLLScene):
    ...
    def create_nll_scene(self, seed: int = 0):
        ...
        # TODO: 1. sample current train data and labels with balanced (how to set the number of samples used in current scene????)
        # TODO:
        ...
        cur_sample_num = 0
        for img_path in self.train_data:
            label = self.train_labels[img_path]
            if (
                sample_num_each_class[label] < (num_samples / self.num_classes)
                and cur_sample_num < num_samples
            ):
        ...
        print(
            f"Clothing1M trainset generated: {len(self.train_labels)} samples in total."
        )
